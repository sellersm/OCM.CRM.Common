...
' Currently the view form is self contained and is not extending the oob interaction form
...
Public Class InteractionExtensionHelper
	...
	Private Property modelView As InteractionExtensionViewFormUIModel
	...
	Public Sub New(ByVal uiModel As UIModeling.Core.RootUIModel, ByVal mode As CRMHelper.FormMode)
		...
		Select Case helperMode
			...
			Case CRMHelper.FormMode.View
				modelView = DirectCast(model, InteractionExtensionViewFormUIModel)
		End Select
	...
	Public Sub InitializeUIDisplay()
		...
		For Each field As UIField In fieldsList
			...
			model.Fields(field.Name).Enabled = False
		Next
	...
	Public Sub UpdateFieldsForSubcategory()
		Dim fieldProperties As InteractionExtensionFieldProperties = Nothing
		Dim interactionSubcategoryID As Guid

		' Currently the view form is self contained and is not extending the oob interaction form, which means there is no parentmodel
		' Therefore, INTERACTIONSUBCATEGORYID is a field on the view form 

		If helperMode = CRMHelper.FormMode.View Then
			interactionSubcategoryID = New Guid(model.Fields(InteractionExtensionFields.INTERACTIONSUBCATEGORYID).ValueObject.ToString)
		Else
		...
		If interactionSubcategoryID <> New Guid("00000000-0000-0000-0000-000000000000") Then
			...
			Using crmSQLConnection = model.GetRequestContext().OpenAppDBConnection()
				...
				For Each field As UIField In fieldsList
					...
					model.Fields(field.Name).Enabled = fieldProperties.isVisible
